...
"""Back up network devices through Nornir/NetBox with inline sanitation.

This is a standalone safe version of the full backup workflow. It collects the
running configuration, environment data, and interface workbook exactly as the
standard backup does, but sensitive running-configuration values are replaced
...
TARGET_TAG = NORNIR_TARGET_TAG
BACKUP_ROOT = Path(
    os.getenv("NORNIR_SAFE_BACKUP_ROOT", "./config_backups_safe")
)
...
def main() -> int:
   ...
   #Checks to confirm if VARs are present/set
    ...
    console.print("\n[bold yellow]--- Starting sanitized configuration backups ---[/]")
    ...
    for host_name in targets.inventory.hosts:
        if host_name in results.failed_hosts:
          console.print(f"[bold red]FAILED[/] {host_name}")
            ...
            console.print(f"        Config:      {config_file.resolve()} (sanitized)")
...
def save_device_outputs(
    ...
    if running_config_result.failed:
        failure_detail = running_config_result.exception
        running_config_result.result = (
            "Running configuration command failed; returned output omitted from logs."
        )
        return failure(
            (
                "Failed to retrieve running configuration: "
                f"{failure_detail or 'device output omitted'}"
            )
        )

    running_config = str(running_config_result.result)

    # Do not allow print_result() or the durable run log to display the raw
    # configuration. The local variable is sanitized before any file write.
    running_config_result.result = (
        "Running configuration collected; contents omitted from logs."
    )

    if not running_config.strip():
        return failure("The device returned an empty running configuration.")

    set_progress(2, "[cyan]Sanitizing configuration[/]")
    try:
        sanitized_config = sanitize_running_config(running_config)
    except SanitizationError as exc:
        return failure(f"Configuration sanitation failed: {exc}", exc)
    ...
    try:
        ...
        config_filename.write_text(
            sanitized_config.rstrip() + "\n",
            encoding="utf-8",
        )
        config_filename.chmod(0o600)
    ...
    return Result(
        ...
        changed=False,
        result=(
            f"Saved sanitized running configuration to "
